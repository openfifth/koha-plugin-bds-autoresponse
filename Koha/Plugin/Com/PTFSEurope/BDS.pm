...
our $metadata = {
    ...
    version         => $VERSION,
    description     => 'Submit isbns to BDS, retrieve results and stage / load to Koha.',
};
our $logger = Koha::Logger->get( { interface => 'intranet', category => 'test' } );
...
sub get_keys_forsubmission {
	...
	my $logfile = $self->retrieve_data('logdir') . $self->retrieve_data('editracefilename');
	my $bds_dir = "Bds/";
	my $date;
	...
	open(my $fh, '<', $logfile) or return { error => "unable to open EDI log file at $logfile" };
	...
	if (@raw_keys) {
		...
		open (my $keys, '>',
		  "${bds_dir}keys/${normalized_date}_keys")
		   or return { error => "Could not open keys_file ${bds_dir}keys/${normalized_date}_keys : $!" };
		...
		if (@isbns) {
			my $filename = $self->retrieve_data('custcodeprefix')  . "${normalized_date}1.TXT";
			my $isnkeysfile = $self->create_input_file( { filename => "Bds/isbns/$filename", keys => \@isbns } );
			if($isnkeysfile->{error}) { 
				return { error => $isnkeysfile->{error}  }; 		                }
		}
		if (@eans) {
			my $filename = $self->retrieve_data('custcodeprefix')  . "T${normalized_date}1.TXT";
			my $eankeysfile = $self->create_input_file( { filename => "Bds/eans/$filename", keys => \@eans } );
...
sub create_input_file {
	...
	open (my $fh, '>', $args->{filename} ) or return { error => "Cannot open $args->{filename}: $!" };
...
sub submit_files {
    ...
    opendir my $dh, $directory or return { error =>  "Cannot opendir $directory: $!" };
    my @submit_files = grep { /^$self->retrieve_data('custcodeprefix')T?\d{9}\.TXT$/ } readdir($dh);
    ...
    if (@submit_files) {
        ...
        if($args->{type} eq "isbns") {
           $ftp->cwd( $self->retrieve_data('upload_isn') )
             or return { error =>  "Cannot change working directory $ftp->message" };
        } else {
...
sub retrieve_files {
    ...
    opendir my $dh,
      "$directory/received" or return { error => "Cannot opendir $directory: $!" };
    my @already_received = grep { /^$self->retrieve_data('custcodeprefix')?\d{9}.?\.mrc$/ } readdir($dh);
    closedir $dh;

    #my $f = $ftp_details->{$type};
    my $ftp = Net::FTP->new( $self->retrieve_data('ftpaddress'), Debug => 0, Passive => 1  )
      or return { error =>  "Cannot connect to  $self->retrieve_data('ftpaddress'): $@" };
    $ftp->login( $self->retrieve_data('login'), $self->retrieve_data('passwd') )
      or return { error => "Cannot login:  $ftp->message" };
    $getf_retval = $self->get_bds_files( { type => $args->{type}, ftp => $ftp, already_received => @already_received } );
    $ftp->quit;
...
sub get_bds_files {
    ...
    foreach my $bdsdirectory (@bdsdirs) {
      $args->{ftp}->cwd( $bdsdirectory )
        or return { error => "Cannot change working directory  $args->{ftp}->message "};
      
      @files_on_server = $args->{ftp}->ls;
      @download_files = grep { /$self->retrieve_data('custcodeprefix')\d{9}.*.mrc$/ } @files_on_server;
...
sub fix_charsets {
	...
	opendir my $dh, $directory or return { error => "Cannot opendir $directory: $!" };
	my @mfiles = grep { /^$self->retrieve_data('custcodeprefix')t\d{9}\.mrc$/ } readdir($dh);
	closedir $dh;
...
sub update_autoresponse {
	...
	open(my $arfh, '>>', $self->retrieve_data('eancontrolmarcfield') . "autoresponse.log") or return { error => "Could not open file " . $self->retrieve_data('eancontrolmarcfield') . "autoresponse.log  $!" };
	...
	foreach my $marcfilename (@marcfiles) {
		...
		while ( my $m = $marcfile->next() ) {
			...
			if ( $marcfilename =~ m/ean/ ) {
				$control        = $m->field($self->retrieve_data('eancontrolmarcfield'));
				$control_number = $control->subfield('a');
			}
			else {
				$control        = $m->field($self->retrieve_data('isncontrolmarcfield'));
			...
			if ( exists $keys->{$control_number} ) {
				...
				$self->update_biblio( { keys => $keys->{$control_number}, m => $m, item_sth  => $item_sth } );
			}
			else {
				say "NO MATCH";
			}
...
sub update_biblio {
    ...
    my $frameworkcode = $self->get_framework( { framework => substr( $leader, 6, 2 ) } );
    ...
    if ($shelfmark) {
        $self->update_shelfmark( { shelfmarl => $shelfmark, biblionumber => $args->{biblionumber}, item_sth => $args->{item_sth} } );
    }
...
sub get_keys {
    ...
    my $k = {};
    opendir my $dh, $directory or return { error => "Cannot open $directory: $!" };
    while ( readdir $dh ) {
        if (/^\d{8}_keys/) {
            my $filename     = "$directory/$_";
            my $new_filename = "$directory/submitted/$_";
            open my $fh, '<', $filename or return { error => "Could not open $filename : $!" };
            while (<$fh>) {
                chomp;
...
sub stage_bds_files {
	...
	my $sql = q|select distinct file_name from import_batches where file_name regexp "$self->retrieve_data('custcodeprefix')[0-9][0-9][0-9][0-9].mrc$" and date(upload_timestamp) > DATE_ADD(CURRENT_DATE(), INTERVAL -6 MONTH)
 order by file_name|;
	...
	open my $fh, '>', $stage_file or return { error =>  "Cannot write to $stage_file : $!" };
...
sub download_new_files {
    ...
    opendir( my $dh, $local_dir ) or return { error => "can't opendir $local_dir: $!" };
    my @loc_files =
      grep { /^$self->retrieve_data('custcodeprefix').*\.mrc$/ && -f "$local_dir/$_" && -M "$local_dir/$_" < 300 }
      readdir($dh);
    closedir $dh;
    ...
    $ftp->binary();
    my $gbdsmresult = $self->get_bds_marc_files( { ftp => $ftp, loc_fil => %loc_fil } );
...
sub get_bds_marc_files {
    ...
    foreach my $bdsdirectory (@bdsdirs) {
      $args->{ftp}->cwd( $bdsdirectory )
      or return { error => "Cannot change working directory $args->{ftp}->message" } ;
    @rem_files = $args->{ftp}->ls($self->retrieve_data('custcodeprefix'). '*.mrc');
...
sub get_potentials {
    ...
    opendir( my $dh, $local_dir ) or return { error => "can't opendir $local_dir: $!" } ;
    my @loc_files =
      grep { /^$self->retrieve_data('custcodeprefix').*\.mrc$/ && -f "$local_dir/$_" && -M "$local_dir/$_" < 160 }
      readdir($dh);
    closedir $dh;
...
sub not_in_archive {
    ...
    if (-f $archive_filename) {
       open my $fh, '<',$archive_filename or return { error => "Cannot open $archive_filename : $!" } ;
       binmode $fh;
       my $archive_digest = Digest::MD5->new->addfile($fh)->hexdigest;
       close $fh;
       my $source_filename = "Source/$args->{filename}";
       open my $fh2, '<',$source_filename or return { error => "Cannot open $source_filename : $!" } ;
       binmode $fh2;
...
sub stage_and_load() {
    ...
    open(my $files_to_stage, '<', "Bds_staging/Inprocess/files_to_stage") or return { error => "Could not open file Bds_staging/Inprocess/files_to_stage $!" };	
	my $file_to_stage="";
	my $batchnumber="";
	while(<$files_to_stage>) {
		chomp $_;
		$file_to_stage= $_;
		system($self->retrieve_data('kohascriptpath') . "stage_file.pl --file Bds_staging/Source/" . $file_to_stage . " --match 1 --item-action ignore > Bds_staging/Logs/" . $file_to_stage . ".log");
		$batchnumber=system("grep '^Batch' " . "Bds_staging/Logs/" . $file_to_stage . ".log | sed 's/[^0-9]//g'" );
		system($self->retrieve_data('kohascriptpath') . "commit_file.pl --batch-number " . $batchnumber . " >> Bds_staging/Logs/" . $file_to_stage . ".log");
...
sub normalize_isbns() {
	my $select_sql = q{select import_record_id, isbn from import_biblios where length(isbn) = 10 and matched_biblionumber is null};

	my $dbh = C4::Context->dbh;

	my $sth = $dbh->prepare('update import_biblios set isbn = ? where import_record_id = ?');
